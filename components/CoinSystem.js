...
export const CoinSystem = () => {
  ...
  return (
    ...
    <div className={`flex items-center justify-center p-5 ${darkMode === true ? 'bg-dark-secondary' : 'bg-light-primary' } rounded-lg shadow-md transition duration-300`}>
      ...
      <button className="px-4 py-2 ml-5 bg-green-600 text-white rounded-md focus:outline-none focus:bg-green-700 transition duration-300 ease-in-out"><Link href="/voucher">Redeem Coins</Link></button>
    </div>
    </div>
  )
}